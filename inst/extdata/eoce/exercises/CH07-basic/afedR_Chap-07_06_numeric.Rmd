...
my_answers <- introR::make_random_answers(my_sol, is_cash = TRUE)
...
Let's assume that, on a certain date, you bought `r total_shares` shares in a company, paying `r introR::format_cash(price_purchase)` per share. After some time, you sold `r qtd_sell_1` shares for a `r introR::format_cash(price_sell_1)` each and the remaining `r qtd_sell_2` shares were sold for `r introR::format_cash(price_sell_2)` on a later day. Using a _script_ in R, structure this financial problem by creating numeric objects. What is the total gross profit/loss from this sequence of transactions on the stock market?
