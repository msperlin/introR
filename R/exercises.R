...
exercises_build <- function(folder_in, type_doc, print_eoce = TRUE) {
  ...
  available_docs <- c("latex", 'epub', 'html')
  if (!type_doc %in% available_docs) {
    cli::cli_abort('wrong type_doc (available = {available_docs}), you chose {type_doc}')
  }
...
exercise_to_text <- function(f_in, my_counter,
  ...
  if (type_doc %in% c('latex', 'epub')) {
    ...
    if (ex_type == "schoice") {
      five_letters <- letters[1:length(alternatives)]
      alternatives <- paste0(five_letters, ") ", alternatives )
...
#' }
exercises_compile_solution <- function(dir_output = "~/introR-solutions",
                                       run_chunks = TRUE) {
