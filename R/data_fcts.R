...
data_list <- function(be_silent = FALSE) {
  ...
  if (!be_silent) {
    ...
    message('')
    cli::cli_alert_success("You can get the local path of file using introR::data_path(name_of_file)")
    cli::cli_alert_success("Example: local_path <- introR::data_path('{sample(my_files, 1)}')")
...
data_path <- function(name_dataset) {
  ...
  if (path_data == '') {
    cli::cli_abort('Cant find name {name_dataset} in list of available tables. Did you check the name with introR::data_list()?"')
  }

  if (!fs::file_exists(path_data)) {
    cli::cli_abort("Cant find file {name_dataset}.. Did you type it correctly? Try using introR::data_list().")
  }
